...
validate_environment() {
    ...
    if [[ ! -f "${APP_BINARY}" ]]; then
        log "WARN" "Application binary not found: ${APP_BINARY}"
        log "INFO" "Please download Cursor IDE from https://cursor.sh/ and place as cursor.AppImage"
        log "INFO" "Continuing with status and configuration operations only"
    fi
...
launch_normal_mode() {
    ...
    # Check if binary exists before launching
    if [[ ! -f "${APP_BINARY}" ]]; then
        log "ERROR" "Cannot launch: ${APP_BINARY} not found"
        log "INFO" "Please download Cursor IDE from https://cursor.sh/"
        return 1
    fi
