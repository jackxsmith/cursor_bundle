#!/usr/bin/env bash
# 10-select_v6.9.32_enhanced.sh — Enhanced Language/Option Selection Script
set -euo pipefail
IFS=$'\n\t'
...
list_languages() {
  ...
  if [[ -f "$SCRIPT_DIR/08-localization_v6.9.32.json" ]]; then
    log "Loading additional languages from localization file..."
    # Extract language codes from JSON file
    grep -o '"[a-z][a-z]"' "$SCRIPT_DIR/08-localization_v6.9.32.json" | sort -u | sed 's/"//g' | while read -r lang; do
