...
# 14-install_v6.9.35.sh — Cursor v6.9.35 Installer (fixed for Ubuntu 25.04)
set -euo pipefail
IFS=$'\n\t'
...
INSTALLER="$BUNDLE_DIR/14-install_v6.9.35.sh"
...
if [[ -f "$BUNDLE_DIR/22-test_cursor_suite_v6.9.35_fixed.sh" ]]; then
  bash "$BUNDLE_DIR/22-test_cursor_suite_v6.9.35_fixed.sh"
fi
