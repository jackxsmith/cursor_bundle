#!/usr/bin/env bash
# Docker Installation Script for Cursor IDE v6.9.34
...
show_help() {
    cat << 'HELP'
Cursor IDE Docker Installer v6.9.34

USAGE:
  ./15-docker_install_v6.9.34.sh [OPTIONS]
...
build_image() {
    ...
    docker build -t cursor-ide:v6.9.34 .
...
remove_all() {
    ...
    docker stop cursor-ide 2>/dev/null || true
    docker rm cursor-ide 2>/dev/null || true
    docker rmi cursor-ide:v6.9.34 2>/dev/null || true
