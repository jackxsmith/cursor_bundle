#!/usr/bin/env bash
# 22-test_cursor_suite_v6.9.35_enhanced.sh — Enhanced Cursor Test Suite
set -euo pipefail
IFS=$'\n\t'
...
test_file_structure() {
  ...
  local required_files=(
    "01-appimage_v6.9.35.AppImage"
    "14-install_v6.9.35_enhanced.sh"
    "02-launcher_v6.9.35_enhanced.sh"
  )
  ...
  if [[ ! -x "$SCRIPT_DIR/01-appimage_v6.9.35.AppImage" ]]; then
...
test_installation() {
  ...
  # Test dry-run first
  if [[ -x "$SCRIPT_DIR/14-install_v6.9.35_enhanced.sh" ]]; then
    log "Testing dry-run installation..."
    if ! "$SCRIPT_DIR/14-install_v6.9.35_enhanced.sh" --dry-run &>>"$LOG"; then
...
test_launcher() {
  ...
  if [[ -x "$SCRIPT_DIR/02-launcher_v6.9.35_enhanced.sh" ]]; then
    log "Testing launcher help..."
    if ! "$SCRIPT_DIR/02-launcher_v6.9.35_enhanced.sh" --help &>>"$LOG"; then
    ...
    if ! "$SCRIPT_DIR/02-launcher_v6.9.35_enhanced.sh" --check &>>"$LOG"; then
...
test_web_ui() {
  ...
  local web_ui_script="$SCRIPT_DIR/06-launcherplus_v6.9.35_fixed.py"
...
test_docker() {
    log "Testing Docker installation method..."
    ...
    if [[ -f "15-docker_install_v6.9.35.sh" ]]; then
        if bash -n "15-docker_install_v6.9.35.sh"; then
        ...
        error "✗ Docker installation script missing"
        return 1
    fi
    ...
    if [[ -f "Dockerfile" ]]; then
        if docker build --dry-run . >/dev/null 2>&1; then
            log "✓ Dockerfile syntax OK"
        else
            warn "Dockerfile may have issues (dry-run not fully supported)"
        fi
    else
    ...
    if [[ -f "docker-compose.yml" ]]; then
        if command -v docker-compose >/dev/null 2>&1; then
            if docker-compose config >/dev/null 2>&1; then
                ...
                warn "Docker Compose configuration may have issues"
            fi
        else
    ...
    log "Docker installation method tests completed"
    return 0
}
...
test_cleanup() {
  ...
  if [[ -x "$SCRIPT_DIR/14-install_v6.9.35_enhanced.sh" ]]; then
    ...
    if "$SCRIPT_DIR/14-install_v6.9.35_enhanced.sh" --uninstall --dry-run &>>"$LOG"; then
