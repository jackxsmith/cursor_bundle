...
#!/usr/bin/env bash
# Full Enterprise Test Suite for Cursor v6.9.32
set -euo pipefail
IFS=$'\n\t'
VERSION="6.9.32"
...
test_file_launcher() {
  echo "[INFO] Testing file 01-appimage_v6.9.32.AppImage"
  if ! test -f "01-appimage_v6.9.32.AppImage"; then
    echo "[ERROR] File check failed: 01-appimage_v6.9.32.AppImage" >> "$ERROR_LOG"
  else
    echo "[INFO] Validating AppImage presence" "01-appimage_v6.9.32.AppImage"
  fi
}

test_file_launcher_sh() {
  echo "[INFO] Testing file 02-launcher_v6.9.32.sh"
  if ! bash -n "02-launcher_v6.9.32.sh" 2>/dev/null; then
    echo "[ERROR] File check failed: 02-launcher_v6.9.32.sh" >> "$ERROR_LOG"
  fi
}

test_file_autoupdater() {
  echo "[INFO] Testing file 03-autoupdater_v6.9.32.sh"
  if ! bash -n "03-autoupdater_v6.9.32.sh" 2>/dev/null; then
    echo "[ERROR] File check failed: 03-autoupdater_v6.9.32.sh" >> "$ERROR_LOG"
  fi
}

test_file_secure() {
  echo "[INFO] Testing file 04-secure_v6.9.32.sh"
  if ! bash -n "04-secure_v6.9.32.sh" 2>/dev/null; then
    echo "[ERROR] File check failed: 04-secure_v6.9.32.sh" >> "$ERROR_LOG"
  fi
}

test_file_secureplus() {
  echo "[INFO] Testing file 05-secureplus_v6.9.32.sh"
  if ! bash -n "05-secureplus_v6.9.32.sh" 2>/dev/null; then
    echo "[ERROR] File check failed: 05-secureplus_v6.9.32.sh" >> "$ERROR_LOG"
  fi
}
...
test_file_policies() {
  echo "[INFO] Testing file 21-policies_v6.9.32.txt"
  if ! grep -Ec "^[1-9][0-9]?\. " "21-policies_v6.9.32.txt" >/dev/null 2>&1; then
    echo "[ERROR] File check failed: 21-policies_v6.9.32.txt" >> "$ERROR_LOG"
  fi
}
