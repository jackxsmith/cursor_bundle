#!/usr/bin/env bash
# bump_and_merge_v6.9.65_keepbranch.sh
# (identical to combined version, but ensures the release branch is re‑pushed if GitHub deletes it)
...
if (( ahead == 0 )); then
  ...
  if merge_cmd --auto; then MERGED_OK=true; else echo "Auto‑merge blocked – retrying plain merge"; merge_cmd && MERGED_OK=true; fi
...
if $MERGED_OK; then
  git fetch origin main; git checkout main; git pull --ff-only origin main; git push origin main; git checkout "$TARGET"
else
  echo "PR did not merge – main unchanged."
fi

# --- branch retention ----
if ! git ls-remote --heads origin "$TARGET" | grep -q "$TARGET"; then
  echo "Branch $TARGET deleted by GitHub – restoring …"
  git push origin "$TARGET"
else
  echo "Branch $TARGET present on remote."
fi

echo "✔ bump & merge flow finished – main current, release branch retained."
