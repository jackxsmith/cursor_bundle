...
# • Run lint (Ruff and ShellCheck) and store results in `lint_report_v6.9.58.txt`.
# • Copy the original bundle archive into the repository as `original_bundle.zip`.
# • Generate diff, README, git log, extended git metadata, webhook template,
#   and a suite of diagnostic files:
#   - test results
#   - build logs
...
#   - CI workflow tarball
#   - code metrics (file and line counts)
#   - TODO/FIXME report (fixed to avoid self‐scanning)
...
# Code metrics: file and line counts by extension
...
# TODO/FIXME scan (exclude the output file itself)
