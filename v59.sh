...
#   and comprehensive diagnostics (tests, builds, static analysis, deps, env,
#   change summary, performance placeholder, CI workflows, metrics, TODO/FIXME,
#   largest files, security audit).
# • Commit everything on `release/v6.9.59`, tag `v6.9.59`, and push both.
#
...
# Policies v6.9.59
* Always create a branch named `release/<version>` and a tag named `v<version>`.  Do not reuse the same name for both.
* Consolidate all patterns in `find` expressions on a single line to avoid unmatched parentheses.
* Align here‑doc delimiters (e.g. `<<EOF`) at column 1 so variables expand correctly and there are no syntax errors.
* Exclude the output file (e.g. `todo_fixme`) and relevant directories from search operations to prevent self‑scanning or long runtime.
* Ensure every `if` is closed with a corresponding `fi`.  Do not use `end`.
* Write scripts to disk using safe tools (e.g. `printf` or `python`) rather than `cat <<EOF` in a shell, to avoid accidental execution during creation.
* Check tool availability before invoking them (pytest, npm, bandit, safety, etc.) and report when they’re not installed.
* Exactly one artefact/log is kept for each logical file, suffixed `_v6.9.59`.  Older duplicates (v6.9.35–58) are removed.
...
* Commits are made on a new branch named `release/v6.9.59` and the tag `v6.9.59` is pushed to `origin`.
* The full suite of diagnostics (diff, logs, metadata, dependencies, environment, tests, builds, static analysis, performance placeholder, CI workflows, README, webhook template, code metrics, TODO/FIXME, largest files, and security audit) is generated and committed with this branch for debugging and historical inspection.
EOF
...
- **git_log_${NEW_VERSION}.txt** – recent history snapshot (decorated commits and stats).
- **git_metadata_${NEW_VERSION}.txt** – detailed Git metadata (logs, refs, remotes, config).
- **webhook_config_v${NEW_VERSION}.json** – template for configuring a webhook on GitHub.
- **test_results_v${NEW_VERSION}.txt** – output of available test suites or a note if none were run.
...
- **dependencies_v${NEW_VERSION}.txt** – snapshots of project dependencies (pip/npm), if available.
- **environment_v${NEW_VERSION}.txt** – system and tool versions captured during the upgrade.
- **change_summary_v${NEW_VERSION}.txt** – list of commit messages since the previous tag.
- **performance_v${NEW_VERSION}.txt** – placeholder for performance or profiling data.
- **ci_workflows_v${NEW_VERSION}.tar.gz** – tarball of CI workflow definitions (if present).
...
# TODO/FIXME report (excluding output file itself)
