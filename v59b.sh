...
# Ensure git is initialised and user identity is set
...
* Consolidate all patterns in `find` expressions on one line to prevent unmatched parentheses.
* Align here‑doc delimiters at column 1 so variables expand correctly and so scripts can be parsed reliably.
* Exclude output files and irrelevant directories from scans (e.g. TODO/FIXME) to prevent self‑scanning.
* Close every `if` with a matching `fi`; do not use `end`.
* Write scripts using safe tools (e.g. `python`, `printf`) rather than `cat <<EOF` to avoid executing commands while creating scripts.
* Check tool availability before running tests, builds, security audits, static analyses, etc., and report if missing.
...
* The script is idempotent: it skips moves if the source and destination are the same and verifies file existence before acting.
...
- **build_log_v${NEW_VERSION}.txt** – build process logs, if available.
...
# CI workflows (always new file with current version name)
...
# TODO/FIXME report (exclude its own file)
