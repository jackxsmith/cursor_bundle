...
# Purpose
# -------
# • Bump project from 6.9.59 → 6.9.64.
# • Remove obsolete or duplicate artefacts, keeping exactly one copy per logical
#   file suffixed _v6.9.64, while preserving all files starting with `ci_workflows_`.
# • Update version strings before renaming.
# • Run lint (Ruff + ShellCheck) and store results in `lint_report_v6.9.64.txt`.
...
# • Commit on `release/v6.9.64`, tag `v6.9.64`, and push both to origin.
...
* Keep exactly one artefact/log per logical file with suffix `_v6.9.64`, removing older duplicates—except for all files starting with `ci_workflows_`, which are preserved.
...
* Lint results are written to `lint_report_v6.9.64.txt`.
* Commits are made on `release/v6.9.64` and tagged `v6.9.64`.
