...
# Purpose
# -------
# • Bump project from 6.9.53 → 6.9.54.
# • Remove obsolete or duplicate artefacts, leaving exactly one copy per logical
#   file (the latest, suffixed _v6.9.54) and deleting superseded logs,
...
# • Commit all changes on `release/v6.9.54`, tag `v6.9.54`, and push to origin.
#
...
find . -type f \( -name "*_v6.9.3[5-9]*" -o -name "*_v6.9.4[0-9]*" -o -name "*_v6.9.5[0-3]*" \) \
  | while read -r f; do
...
# Policies v6.9.54
* Exactly one artefact/log is kept for each logical file, suffixed `_v6.9.54`.  Older duplicates (v6.9.35–53) are removed.
...
* Commits are made on a new branch named `release/v6.9.54` to avoid naming conflicts with the tag `v6.9.54`.  Both the branch and the tag are pushed to the `origin` remote.
